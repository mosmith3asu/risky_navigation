...
class DDPGAgent:
    ...
    def __init__(self, env,
        ...
        self.ou = OUNoise(self.action_dim, theta=ou_theta, sigma=ou_sigma, sigma_decay=ou_sigma_decay,
...
class DDPGAgentRationalProspects(DDPGAgent):
    """
    Vectorized DDPG. Mirrors your A2CAgentVec patterns (env is Sync/AsyncVectorEnv).
    """
    ...
    def warmup(self):
        ...
        for ep in range(self.warmup_epis):
            ...
            while i <= self.env.get_attr('max_steps')[0]:
                ...
                # Step envs ############################################
                a = np.random.uniform(self.a_low.cpu().numpy(), self.a_high.cpu().numpy(),size=(1,self.action_dim)).astype(np.float32)
                ns_true, _, done, _ = self.base_env.step(a)  # step the base env for rendering reference
                ns_samples, r_samples, terminated, truncated, info = self.env.step(np.repeat(a[np.newaxis, :], self.num_envs, axis=0))

                # Ending observation/state ############################################
                self.env.call('update_delay_buffers', current_robot_state=prev_state[:4].copy(), action=a)
                o2 = self.env.call('observe')[0]  # Consistent across all possibilities give buffer > 1
                # o2 = self.env.call('observe_latest')[0]  # Consistent across all possibilities give buffer > 1

                # Create transition prospects and store in mem ################################
                r_probs = np.array(self.env.get_attr('robot_prob'))[1:]
                r_prospects = np.vstack([r_samples, r_probs])
                # dones = terminated or truncated

                self.replay.add(o1, a, r_prospects, o2, done) #TODO: true_done or dones?
    ...
    def risk_measure(self,vals,probs):
        return torch.sum(vals*probs, dim=0) # rational expectation
