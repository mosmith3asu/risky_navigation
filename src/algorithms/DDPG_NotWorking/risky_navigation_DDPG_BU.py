...
class DDPGAgent(object):
    def __init__(self, state_dim, action_dim, max_action,min_action, device,
                 lr = 1e-3,tau= 1e-4,gamma=0.97,batch_size=128):
        self.device = device
        self.max_action = max_action
...
class Logger:
    ...
    def log(self,ep_reward,state_seq,terminal_cause):
...
# Training loop
def train(env, test_env,agent, episodes=500, max_steps=1000,test_interval=10,test_episodes=10):
    rand_start_dur = 500
    rew_shape_dur = 500
    ...
    for ep in range(episodes):
        ...
        logger.log(ep_reward, trajectories, terminal_cause)
            ...
            if not proc.is_alive() or not queue.empty():
                ...
                if len(results)> 0:
                    for i in range(len(results[0])):
                        ...
                        logger.log(rew, tr, terminal_cause)
...
def main():
    ...
    scores = train(env,test_env, agent, episodes=1000, max_steps=env.max_steps)
    env.close()
